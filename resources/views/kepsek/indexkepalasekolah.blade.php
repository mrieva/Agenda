            ...
            fetch(`{{ route('kehadiran.chart') }}?time_filter=${timeFilter}`)
                ...
                .then(data => {
                    var dataGuru = google.visualization.arrayToDataTable([
                        ['kehadiran', 'Jumlah'],
                        ['Hadir', data.guru.find(item => item.kehadiran === 'hadir')?.jumlah || 0],
                        ['Izin', data.guru.find(item => item.kehadiran === 'izin')?.jumlah || 0],
                        ['Sakit', data.guru.find(item => item.kehadiran === 'sakit')?.jumlah || 0]
                    ]);
                    ...
                    var dataSiswa = google.visualization.arrayToDataTable([
                        ['kehadiran', 'Jumlah'],
                        ['Hadir', data.siswa.find(item => item.kehadiran === 'hadir')?.jumlah || 0],
                        ['Izin', data.siswa.find(item => item.kehadiran === 'izin')?.jumlah || 0],
                        ['Sakit', data.siswa.find(item => item.kehadiran === 'sakit')?.jumlah || 0]
                    ]);
